// mirrorGame.cpp
// An implemented game child class used to play Game Of Life on Mirror mode given an initial board.
...
MirrorGame::MirrorGame(int rows, int columns, string initialBoard)
{
...
void MirrorGame::Simulation()
{
  ...
  // matrix. Since this is MIRROR MODE, I wil fill these extra cells as reflections of the most outer cells on the start matrix.
  for (int i = (m_play_matrix_minimum_row_index); i <= m_play_matrix_maximum_row_index; ++i)
  {
    for (int j = (m_play_matrix_minimum_column_index); j <= m_play_matrix_maximum_column_index; ++j)
    {
      ...
      {
        play_matrix_1[i][j] = start_matrix[m_start_matrix_minimum_row_index][m_start_matrix_minimum_column_index];
      }
      else if (i == m_play_matrix_maximum_row_index && j == m_play_matrix_minimum_column_index)
      {
        play_matrix_1[i][j] = start_matrix[m_start_matrix_maximum_row_index][m_start_matrix_minimum_column_index];
      }
      else if (i == m_play_matrix_minimum_row_index && j == m_play_matrix_maximum_column_index)
      {
        play_matrix_1[i][j] = start_matrix[m_start_matrix_minimum_row_index][m_start_matrix_maximum_column_index];
      }
      else if (i == m_play_matrix_maximum_row_index && j == m_play_matrix_maximum_column_index)
      {
        play_matrix_1[i][j] = start_matrix[m_start_matrix_maximum_row_index][m_start_matrix_maximum_column_index];
      }
      else if (i == m_play_matrix_minimum_row_index)
      {
        play_matrix_1[i][j] = start_matrix[m_start_matrix_minimum_row_index][j-1];
      }
      else if (i == m_play_matrix_maximum_row_index)
      {
        play_matrix_1[i][j] = start_matrix[m_start_matrix_maximum_row_index][j-1];
      }
      else if (j == m_play_matrix_minimum_column_index)
      {
        play_matrix_1[i][j] = start_matrix[i-1][m_start_matrix_minimum_column_index];
      }
      else if (j == m_play_matrix_maximum_column_index)
      {
        play_matrix_1[i][j] = start_matrix[i-1][m_start_matrix_maximum_column_index];
      }
      else
      {
